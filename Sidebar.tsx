...
// Fixed relative path to types.ts as this file is in the root
...
// Fixed relative path to components/AboutSection
      ...
      <div className="h-10 bg-zinc-900/50 rounded-lg overflow-hidden border border-white/5 flex items-end gap-[2px] p-1.5">
        {bars.map((height, i) => (
          <div 
            ...
            style={{ height: `${active ? height : 5}%`, opacity: active ? 0.4 + (height/200) : 0.2 }}
          />
        ))}
      </div>
    </div>
...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  // Computed stream name based on activeStream selection
